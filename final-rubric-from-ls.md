# Final Grading Rubric

**Course:** IS 590r – AI Applications in Business  
**Total Points:** 200 (weighted across three grading domains)  
**Philosophy:** We grade your **PROCESS**, not your product. A team with a simpler app and a strong, documented process will outscore a team with a polished product and no evidence of how they built it.

---

## What Changed from Midterm


| Midterm Allowance                                    | Final Expectation                                                                                                                                                                                                                                            |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| No penalty for missing code/implementation           | Working demo required — in person. Code must exist and run.                                                                                                                                                                                                  |
| Friends-and-family interviews accepted               | Must include interviews outside your immediate social circle — target users, strangers, or domain contacts. Friends-and-family can supplement but cannot be the only source.                                                                                 |
| Falsification tests designed but unexecuted          | Falsification tests must be executed with documented results.                                                                                                                                                                                                |
| Empty changelogs, stale docs not penalized           | Documents must be living artifacts — current `context.md`, updated roadmaps, PRD that reflects the actual project. If you pivoted, version history should show the evolution.                                                                                |
| Logging library present but unused in app            | Structured logging must be integrated into the actual application, not just defined in a standalone file.                                                                                                                                                    |
| 15-minute presentations, no demo required            | 20-minute presentations with in-person working demo and process narrative.                                                                                                                                                                                   |
| Midterm was a first look — introduce your idea       | Final is a continuation, not a repeat. Don't re-present your midterm. Start from where midterm left off and show what changed, deepened, or pivoted.                                                                                                         |
| MCP scored as a sub-criterion                        | MCP is no longer a scored criterion. MCP configs often contain secrets and should be gitignored. We care that you used MCP, not that the config is committed.                                                                                                |
| `CLAUDE.md` / `.cursorrules` not explicitly required | `CLAUDE.md` or `.cursorrules` with behavioral guidance is now an explicit sub-criterion in AI Development Infrastructure.                                                                                                                                    |
| Some teams gitignored their `ai/` folder             | **Do NOT** gitignore your `ai/`, `aiDocs/`, or planning folders. Graders must be able to see your roadmaps, plans, changelogs, and `context.md`. Only gitignore secrets and library folders (`.env`, `.testEnvVars`, MCP configs, `node_modules/`, `venv/`). |
| `mvp.md` not consistently graded at midterm          | `mvp.md` is a required deliverable for the final. It defines the concrete, scope-constrained version of what you are actually building. The PRD can include broader vision and future features, but `mvp.md` is the anchor for what you are delivering now.  |
| No peer evaluation                                   | Confidential peer evaluation form is a required deliverable (see separate form). Peer evaluations do not directly change scores but may be used to adjust individual grades if significant contribution imbalances are identified.                           |


**Important:** Everything that was expected at midterm is still expected. These are additions, not replacements.

---

## Grading Philosophy: Process Adherence as a Lens

Process adherence is not a separate grading area -- **it is the lens through which ALL areas are evaluated**. Students are expected to demonstrate that they followed the processes taught in class: document-driven development, AI-augmented workflows, systematic iteration, and structured debugging. This is assessed within each technical and product area, not scored as its own category. Course material is required process, not optional suggestions.

---

## Who Grades What


| Grader       | Focus                   | Weight | Areas                                                        | Scoring                            |
| ------------ | ----------------------- | ------ | ------------------------------------------------------------ | ---------------------------------- |
| Jason        | Product & system design | 45%    | 5 areas, each scored out of 20 points (100 raw total)        | Weighted to 90 of 200 final points |
| Casey        | Technical process       | 45%    | 4 areas, each scored out of 25 points (100 raw total)        | Weighted to 90 of 200 final points |
| Guest Grader | Presentation quality    | 10%    | 4 sub-criteria, each scored out of 25 points (100 raw total) | Weighted to 20 of 200 final points |


---

### Jason's Product & System Design (45% of Final Grade)

Each area is scored out of **20 points.**  
Total: 100 raw points, weighted to 90 of 200 final points.

At midterm, we graded whether you had these artifacts — system diagram, problem statement, customer definition, success criteria, evidence of customer conversations. At final, we grade **what happened when those artifacts met reality.**  
If your final presentation sounds identical to your midterm, you haven't demonstrated growth.

#### 1. System Understanding (20 points)

At midterm you showed us a system diagram. At final, show how *building your product* changed your understanding of the system.

- Show what you got wrong — or what you didn't see — at midterm.
- What new elements, relationships, or feedback loops did you discover by actually operating in this system?
- Which leverage points did you try to pull, and what happened?
- *Your diagram should look different from midterm because your understanding is different.*

#### 2. Problem Identification (20 points)

At midterm you identified a problem. At final we want to see how that problem was **tested, refined, and sharpened** through real-world experience.

- Did your falsification tests confirm or challenge your assumptions?
- If you pivoted, walk us through the evidence that made you change.
- If you stayed the course, show what **validated your hypothesis** — not just "we still believe it," but "here's what we learned that proved it."
- *Your problem statement should be more precise and grounded than at midterm.*

#### 3. Customer Focus (20 points)

At midterm you identified a target customer. At final, show how your understanding of that customer deepened through **real research and engagement**.

- Did you talk to people beyond your friends and family — actual target users, domain experts, strangers?
- Has your competitive analysis evolved?
- *Solution positioning should reflect actual feedback, not just assumptions.*

#### 4. Success & Failure Planning (20 points)

At midterm you defined what success and failure would look like. At final:

- Did you actually **measure** against those criteria?
- Where do you stand against the metrics you set?
- If you couldn't measure as planned, what did you learn about measurement itself?
- *Pivot plans should be informed by real data, not just hypotheticals.*

#### 5. Customer Interaction (20 points)

At midterm you showed that you talked to customers. At final, show that those conversations **shaped what you built**.

- Point to features, design decisions, or pivots that exist due to customer feedback.
- Demonstrate the feedback loop: engage → learn → change → re-engage.
- *Interviews must extend beyond friends and family.*

---

### Casey's Technical Process (45% of Final Grade)

Each area is scored out of **25 points**.  
Total: 100 raw points, weighted to 90 of 200 final points.


| #   | Area                                 | Points | What "Good" Looks Like                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --- | ------------------------------------ | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | PRD & Document-Driven Development    | 25     | PRD is clear and actionable. A separate mvp.md defines the concrete deliverable. PRD evolves (with version history) if you pivot. Development follows PRD → mvp.md → plan → roadmap → implementation. Documents are living and current. Evidence of AI-assisted iteration, not just one-shot generation.                                                                                                                                        |
| 2   | AI Development Infrastructure        | 25     | AI folder pattern implemented (`context.md`, project docs). Context.md uses the bookshelf pattern (references key docs with 1-2 sentence descriptions), is up-to-date. `CLAUDE.md` or `.cursorrules` provides behavioral guidance. Git workflow shows branching, meaningful commits, and PRs. ai/ folder is committed (graders need to see all planning artifacts). `.gitignore` covers secrets and library folders only. No secrets committed. |
| 3   | Phase-by-Phase Implementation & Demo | 25     | Code built incrementally following roadmap phases. Roadmaps used as checklists; tasks checked off. Evidence of multi-session development (plan/implement/review). Git history shows incremental progress. In-person working demo demonstrating core functionality.                                                                                                                                                                              |
| 4   | Structured Logging & Debugging       | 25     | Structured logging integrated into application code. CLI test scripts exist and work. Test-log-fix loop followed. Debugging process is documented or evidenced in git history.                                                                                                                                                                                                                                                                  |


---

### Guest Grader (10% of Final Grade)

Assesses **presentation quality** using 4 sub-criteria, each scored out of 25 points (100 raw total), weighted to 20 of 200 final points.


| #   | Sub-Criterion          | Points | What "Good" Looks Like                                                                                                      |
| --- | ---------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------- |
| 1   | Communication Quality  | 25     | Presentation is clear, confident, and well-delivered. All team members speak competently. Q&A responses are thoughtful.     |
| 2   | Storytelling & Journey | 25     | Team tells a compelling project story. Honest self-awareness of challenges and improvements. Growth since midterm is clear. |
| 3   | Visual Design & Demo   | 25     | Slides/visuals support narrative. Demo is well-integrated throughout, not just at the end.                                  |
| 4   | Overall Impact         | 25     | Leaves a positive, professional impression. A listener can easily explain what was built, why, and what was learned.        |


---

## Scoring Scale

For each area, a four-level scale is used. Solid work earns an A-minus.


| Level        | What It Means                                                              | Score Range              |
| ------------ | -------------------------------------------------------------------------- | ------------------------ |
| Exemplary    | Exceptional work that goes beyond expectations. Could be a future example. | 95-100% of area points   |
| Proficient   | Solid work, process clearly followed. Meets all expectations.              | 90-94% of area points    |
| Developing   | Partial implementation; some gaps in process or understanding.             | 80-89% of area points    |
| Insufficient | Major gaps, process not followed, or work not submitted.                   | Below 80% of area points |


---

## Grade Scale


| Grade | Minimum % | Minimum Points (/200) |
| ----- | --------- | --------------------- |
| A     | 93%       | 186                   |
| A-    | 90%       | 180                   |
| B+    | 87%       | 174                   |
| B     | 83%       | 166                   |
| B-    | 80%       | 160                   |
| C+    | 77%       | 154                   |
| C     | 73%       | 146                   |


---

## Worked Scoring Example

Suppose a team receives:


| Grader                          | Raw Score (out of 100) | Weight | Weighted Points  |
| ------------------------------- | ---------------------- | ------ | ---------------- |
| Jason (Product & System Design) | 85                     | 45%    | 85 × 0.90 = 76.5 |
| Casey (Technical Process)       | 90                     | 45%    | 90 × 0.90 = 81.0 |
| Guest Grader (Presentation)     | 92                     | 10%    | 92 × 0.20 = 18.4 |


**Total:** 175.9 / 200  
175.9 / 200 = 87.95% → **B+**

*Jason and Casey's raw scores are multiplied by 0.90 (to map 100 raw → 90 weighted).*  
*Guest Grader's raw score is multiplied by 0.20 (to map 100 raw → 20 weighted).*

---

## Presentation Format

- **Duration:** 20 minutes per team. Prioritize, rehearse, address all required elements briefly.
- **Schedule:** Presentations: April 8, 13, and 15.
- **Submission deadline:** Monday April 7 at 23:59.  
All project code, documentation, and presentation materials must be **committed and pushed** by this deadline, regardless of your presentation day.  
*No exceptions, no late submissions. Work pushed after this deadline will not be graded.*
- **All team members must contribute** and be able to explain their individual contributions.

### Required Elements

- System design diagram
- Process narrative: how you planned, built, iterated, and adapted
- In-person working demo — plan to demo live.  
*A backup recording is recommended in case of technical problems; you won't be penalized if you must use it unexpectedly. Contact Casey or Jason in advance if a live demo truly isn't possible.*
- Show the product throughout the presentation, not just at the end
- Honest discussion of what you learned, what surprised you, and what you'd do differently
- Q&A: Thoughtful, honest responses, demonstrating understanding of trade-offs & limitations
- Project story: Present the full journey — plan, pivots/refinements, process, and results
- **Peer evaluation:** Each team member submits a confidential peer evaluation form (see separate form)

---

## Structuring Your Presentation: Pivot vs. Refinement

*Do not re-present your midterm. Structure according to your team's path:*

**If your team pivoted to a new idea:**

- Open with the pivot story: what evidence triggered the change, how you evaluated a new direction
- Show customer validation of the new idea (not just "we think it's better")
- Demo the new product — how it works, who it serves
- Connect the pivot to falsification results and customer feedback — evidence-driven pivots are a success
- Show updated system diagram, problem statement, and success/failure criteria for the new direction

**If your team refined the original idea:**

- Start from where midterm left off — don't re-introduce your idea
- Show the depth enabled by sustained execution: more customer feedback (outside your circle), deeper problem understanding, validated/invalidated assumptions
- Show evolution in system diagram, problem statement, and competitive analysis
- Demo should show meaningful progress — more functional, more validated, more informed by real feedback
- Demonstrate the iteration loop: engage → learn → change → re-engage

*Either path is equally valid. We grade the quality of your process and the depth of your learning, not whether you pivoted.*

---

## What We're NOT Grading

- A finished, polished product. A working prototype is great; a production-ready app is not expected.
- Paying customers, revenue, or a business model.  
**But:** We DO expect evidence of customer research/engagement — including interviews with people outside your immediate social circle.
- Which AI tool you used.  
(Claude Code, Cursor, ChatGPT — we care about the process, not the brand.)
- Complexity of the idea.  
A simple idea, executed with strong process, beats an ambitious idea with no evidence of how it was built.
- Perfect code.  
We want to see the journey: plans, failures, fixes, iteration.
- Unlimited AI access.  
Efficient use of limited/free-tier resources is a positive signal.
- MCP configuration files in the repo.  
MCP configs often contain secrets and **should be gitignored**. We care that you used MCP in your workflow, not that the config is committed.

---

## Quick Evidence Checklist

*Use this to self-assess before your presentation. You do not need every single item, but the more you can demonstrate, the stronger your grade.*

### Jason's Product & System Design Domain

**System Understanding:**

- System diagram has evolved since midterm — not just cosmetic changes
- Team can articulate what they learned about the system through building
- Leverage points validated through experience, not just hypothesized. 

**Problem Identification:**

- Problem statement has matured — refined or pivoted based on evidence
- Falsification tests executed with documented results that influenced understanding
- If pivoted: evolution justified; if stayed: evidence-based rationale for why

**Customer Focus:**

- Customer understanding deepened through research beyond friends and family
- Competitive analysis updated based on building experience
- Solution positioning reflects validated understanding, not initial assumptions

**Success & Failure Planning:**

- Success/failure criteria tested against reality — team can report where they stand
- Pivot plans informed by real data, not midterm hypotheticals

**Customer Interaction:**

- Continued interaction beyond midterm with clear feedback loop (engage → learn → change → re-engage)
- Features or decisions directly shaped by customer feedback
- Awareness of driving vs. being driven by the customer

---

### Casey's Technical Domain

**PRD & Document-Driven Development:**

- PRD exists and is comprehensive (broader vision is fine)
- `mvp.md` exists and defines the concrete, scope-constrained deliverable
- Development is document-driven (PRD → mvp.md → plan → roadmap → implementation)
- Documents reflect current project state
- Evidence of AI-assisted iteration on documents

**AI Development Infrastructure:**

- AI folder has `context.md`, architecture docs, coding-style docs
- `context.md` uses the bookshelf pattern (references key docs with short descriptions) and is current
- `CLAUDE.md` or `.cursorrules` with behavioral guidance
- Proper Git workflow (branching, meaningful commits; branches don't need to be kept after merge)
- `ai/` folder is committed (not gitignored) — graders can see plans, changelogs, etc.
- `.gitignore` covers secrets and library folders only (`.env`, `.testEnvVars`, MCP configs, `node_modules/`, `venv/`)
- No secrets in the repo

**Phase-by-Phase Implementation & Working Demo:**

- Roadmaps with tasks checked off showing progression
- Git history shows iterative, incremental development (not one big burst)
- Evidence of multi-session workflow (plan/implement/review)
- Roadmaps are living documents
- In-person working demo of core functionality

**Structured Logging & Debugging:**

- Structured logging implemented (not just `console.log("here")`)
- Logging integrated into application code
- CLI test scripts exist and work
- Proper exit codes (0/1/2) on scripts
- Evidence of test-log-fix loop in git history

---

### Presentation

- All project and presentation materials committed and pushed by April 7 at 23:59
- All team members contribute and can explain their role
- System design diagram included
- Process narrative included (not just product demo)
- In-person working demo
- Honest discussion of what was learned and what you would do differently
- Thoughtful Q&A responses showing understanding of trade-offs
- Confidential peer evaluation form submitted

---

## A Note on Iteration, Pivoting, and Failure

This course values **learning through doing**. If your team pivoted, encountered a setback, or realized your original plan was wrong — that **is not a problem; it's evidence of good process**. Show us the pivot. Explain what you learned. That story is often more compelling than a smooth path from start to finish.

A team that stayed the course and executed well on the original plan is **not penalized** for sticking with it.

**We are not grading whether you pivoted — we are grading the quality of your process regardless of which path you took.**