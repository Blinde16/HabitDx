      ...
      <View className="px-6 py-8">
                  ...
                  <View className="flex-1">
                    <Text 
                      className={`text-lg font-bold ${
                        habit.status === 'completed' ? 'text-gray-500 line-through' : 'text-gray-900'
                      }`}
                    >
                      {habit.name}
                    </Text>
                    {habit.status === 'completed' && habit.checked_in_at && (
                      <Text className="text-xs text-gray-500 mt-1">
                        Completed at {new Date(habit.checked_in_at).toLocaleTimeString('en-US', {
                          hour: 'numeric',
                          minute: '2-digit',
                        })}
                      </Text>
              ...
              {habit.status === 'completed' && (
                <View className="bg-yellow-50 rounded p-3 mt-2">
                  <Text className="text-sm text-gray-800">
                    <Text className="font-semibold">🎉 Don't forget:</Text> {habit.celebration}
                  </Text>
                </View>
              )}
              ...
              {habit.status !== 'completed' && habit.streak === 0 && habit.last_obstacle && (
                <View className="mt-2 bg-orange-50 border-l-4 border-orange-500 rounded p-3">
                  ...
                  <Text className="text-xs text-orange-800">
                    You missed yesterday. One skip is fine—but two in a row starts a pattern. Let's
        ...
        {totalCount > 0 && (
          <View className="bg-blue-50 rounded-lg p-4 mt-4">
            ...
            <Text className="text-sm text-gray-800">
              Don't miss twice! One skip is fine—life happens. It's two in a row that starts a
              pattern. Focus on consistency over perfection.
            </Text>
          </View>
        )}
      </View>
